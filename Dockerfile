...
ARG RUST_VERSION=nightly-2022-07-01
ARG APP_NAME=lambda
...
RUN apk add --no-cache clang lld musl-dev git file ca-certificates
...
cp ./target/$(xx-cargo --print-target-triple)/release/$APP_NAME /app/server
xx-verify /app/server
EOF
...
COPY --chown=appuser:appuser --from=build /app/server ./
# Copy initial backend configs if any
COPY --chown=appuser:appuser ./deploy/data ./data/
